...
class StudentManagementApp:
    ...
    # -------------------------
    # AI Prediction Feature (Grade + Comment + Avg)
    # -------------------------
    def ai_predict_grade(self, student):
        try:
            model_path = "grade_predictor.pkl"
            ...
            marks = [float(m) for m in student.marks if m != ""]
            if not marks:
                messagebox.showwarning("No Marks", "No marks available for prediction.")
                return

            avg = np.mean(marks)
            pred_num = model.predict([[avg]])[0]
            ...
            if pred_num >= 2.5:
                ...
                grade = "B"
                comment = "Good work, but you can aim even higher 💪"
            else:
                grade = "C"
                comment = "Needs improvement — stay consistent and seek help where needed 📘"
            
            messagebox.showinfo(
                "🤖 AI Grade Prediction",
                f"Current Average: {avg:.2f}\nPredicted Grade: {grade}\n\n{comment}\n\n(AI model based on past student data)"
            )
        except Exception as e:
            messagebox.showerror("Error", f"AI prediction failed:\n{e}")
